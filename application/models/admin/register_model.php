...
class Register_model extends CI_Model {
	...
	function cekDataNIS() {
		...
		// query
		$where = ['nis' => $nis, 'verifikasi_alumni' => "null"];
		...
		if ( $query->num_rows() == 1 ) {
			...
			// query
			$where = ['nis' => $nis];
			$query = $this->db->get_where('profil_siswa', $where)->row_array();

			if ( $query['verifikasi_alumni'] == "diterima" ) {

				$pesan = "Tidak dapat mengajukan karena telah diterima sebagai Alumni";
			} else {

				$pesan = "Pengajuan anda sedang kami proses ";
			}
