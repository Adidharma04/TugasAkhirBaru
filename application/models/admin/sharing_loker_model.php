...
class Sharing_loker_model extends CI_Model {
    ...
    public function editDataInformasiLoker( $id_loker){
       ...
       if ( !empty( $_FILES['foto']['name'] ) ) {
           ...
           if ( $this->upload->do_upload('foto') ){
               ...
               redirect('admin/sharing_loker/edit/'. $id_loker);
       ...
       if ( !empty( $_FILES['berkas']['name']) ) {
           ...
           if ( $upload_berkas['result'] == "success" ) {
               ...
               if ( $ambilInformasiLoker->berkas ) { // remove old document
                   ...
                   $link = './assets/Gambar/Upload/Loker/'. $ambilInformasiLoker->berkas;
                   unlink( $link );
               }
           }
       } else {
    ...
    function prosesHapusLoker( $id_loker ){
        ...
        // hapus file lama 
        if ( $ambilInformasiLoker->foto ) { // remove old document

            $link = './assets/Gambar/Upload/Loker/'. $ambilInformasiLoker->foto;
            unlink( $link );
        }
        if ( $ambilInformasiLoker->berkas ) { // remove old document
